...
function getFieldsForCurrentAndPassFail(row) {
    ...
    if(row.showResults == 'Y') {
        if((typeof row.simulation_score !== 'undefined' && row.simulation_score != null) && (typeof row.compXM_score !== 'undefined' && row.compXM_score != null)){
                let isSimulationNumber = !isNaN(parseInt(row.simulation_score))
                let isCompXMNumber = !isNaN(parseInt(row.compXM_score))
                TEE.value = (isSimulationNumber && isCompXMNumber) ? "SIM-"+row.simulation_score + "/" + row.simulation_max_score + " | CXM-" +row.compXM_score + "/" + row.compXM_max_score : row.teeScore + "/" + row.teeScoreMax
                TEE.class = (isSimulationNumber && isCompXMNumber) ? row.teeIsPass == 'Y' ? green : red : formatAndReturnMarks(row.teeScore)
                
        }else if(typeof row.teeScore !== 'undefined') {
