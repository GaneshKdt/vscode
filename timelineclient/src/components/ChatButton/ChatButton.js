...
import {faComments,faPlus,faWindowClose}from '@fortawesome/free-solid-svg-icons'
...
class ChatButton extends Component
{
    constructor(props) {
        super(props)
        this.state = {
            quickBloxUrl : API.quickbloxUrl+"?sapid="+props.sapid+"&firstName="+props.firstName+"&lastName="+props.lastName+"&isAdmin=N",
            showChat : false,
            showModal : false,
            hideModal : true,
        }
        this.handleChatDisplay = this.handleChatDisplay.bind(this) 
        this.initializeChat = this.initializeChat.bind(this) 
        this.showOfflineAlert = this.showOfflineAlert.bind(this) 
    ...
    return(
        <>
        <div>
            <div id="liveagent_button_online_57390000000H2q5" className='chat-online' onClick={()=>this.initializeChat(this.props.liveagent)}>
              <FontAwesomeIcon className="fa-lg" icon={faComments} />
            </div>
            <div id="liveagent_button_offline_57390000000H2q5" style={{display:"none"}} className='chat-offline' onClick={() => this.showOfflineAlert()}>
              <FontAwesomeIcon className="fa-lg" icon={faComments} />
            </div>
        </div>
