export const tabs = [
    ...
    { key: 'select-center-slot',    id : 1, label : 'Select Slot and Center' },
    { key: 'confirm-selection',     id : 2, label : 'Confirm Selection'      },
    { key: 'select-payment-option', id : 3, label : 'Select Payment Options' },
    { key: 'submit-form',           id : 4, label : 'Payment'                },
]
...
export function nextButtonDisabled(currentTab, selectedSubjects, selectedPaymentProvider) {
    ...
    if(currentTab >= getIdByKey('select-center-slot')) {
        
        enabled = enabled && slotsSelected
    }
    if(currentTab >= getIdByKey('confirm-selection')) {
        enabled = enabled && slotsSelected && subjectsSelected
    }
