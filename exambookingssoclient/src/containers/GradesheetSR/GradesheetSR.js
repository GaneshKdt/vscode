...
//         url : API.getAvailableMarksheets,
//         data : { sapid: props.sapid },
...
//     if(this.checkDuplicateRequests() === "duplicate" || this.checkSelectsSelected() === false){
//         return false;
//     }
//     else{
...
//     <BootstrapTable keyField='id' data={ responseData } columns={ column }  
//     >
...
//         if(tempExamDetails[i]!== "" && tempExamDetails[i+1]!== "" && tempExamDetails[i] === tempExamDetails[i+1]){
//             duplicateRecords[i] = tempExamDetails[i];
//         }
//     }
//     console.log("inside duplicate------222---",duplicateRecords);
...
// export default connect(mapStateToProps)(withRouter(GradesheetSR));
...
import { API } from '../../shared/config'//import {analyticsManager} from '../../../shared/Analytics'
...
class GradesheetSR extends Component {
    constructor(props) {
        super(props);
    }
    state = {
        ...
        // exam attributes
        examYearList : ["2019","2020","2021"],
        examMonList : ["Dec", "Mar", "Jun" ,"Sep"],
        semList : ["1","2", "3", "4", "5"],
    ...
    getClearedSemForStudent = () =>{
        ...
        axios.defaults.headers.post['Content-Type'] = false;
        axios.post(urls.apiUrl_exam + "getClearedSemForStudentMBAX",
        {
            "sapid" : this.state.sapId,
        }
        ).then(response =>{
                ...
                if(Object.entries(this.state.errors).length === 0){
                    ...
                    confirmAlert({
                        ...
                        buttons: [
                        {
                            ...
                            onClick: () => {
                                axios.defaults.headers.post['Content-Type'] = false;
                                axios.post(urls.apiUrl_studentPortals + "/checkMarksheetHistoryForMBAX",
                                {
                                    // actual data
                                    sapId : this.state.sapId,
                                    serviceRequestType : this.state.serviceRequestType,
    ...
    handlePreviewMarks = (evt) =>{
        ...
        axios.defaults.headers.post['Content-Type'] = false;
        axios.post(urls.apiUrl_exam + "/generateMarksheetPreviewFromSR_MBAX",
        body,
        ).then(response =>{
            console.log(JSON.stringify(response));
            this.setState({
    ...
    checkDuplicateRequests() {
        ...
        console.log("inside duplicate----111-----",tempExamDetails);
        for(var i =0; i<tempExamDetails.length; i++){
            if(tempExamDetails[i]!== "" && tempExamDetails[i+1]!== "" && tempExamDetails[i] === tempExamDetails[i+1] &&  tempExamDetails[i]!=undefined ){
                duplicateRecords[i] = tempExamDetails[i];
                console.log(i+" "+tempExamDetails[i]);
            }
        }
        console.log("inside duplicate------222---",duplicateRecords+"length is "+duplicateRecords.length);
        if(duplicateRecords.length > 0){
            alert("Kindly verify the current selection for Marksheet Request.Possible duplicate values");
            return "duplicate";
        }
            ...
            <Card style={{maxWidth : "80%"}}>
                    ...
                    <Card.Footer>
                        ...
                        {this.state.forward === true?
                            <Redirect to={{pathname:'/ssoservices/mbax/marksheetSummary' ,state:{responseData : this.state.responseData}}}  />
                        : null }
                    </Card.Footer>
                </Card>
...
export default connect(mapStateToProps)(GradesheetSR)
