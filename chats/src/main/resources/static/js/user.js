        ...
        return new Promise(function (resolve, reject) {
            ...
            var colback = function (err, responce) {
                ...
                var userFilteredList = responce.items.filter(function(data){
                    ...
                    for(let i=0;i<=chatUserList.length-1;i++)
                    {
                        let userSapid=chatUserList[i].sapid;
                        if(userSapid==data.user.login && userName!='Group Admin')
                        {
                        	return true;
                        }
                    }
...
User.prototype.initGettingUsers = function (userListConteiner, userListFilter, params) {
    ...
    this.userListFilter &&
    this.userListFilter.addEventListener('keydown', this.idle);
